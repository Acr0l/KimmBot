...
const quizVerifier = async (quiz) => {
    subjectsArray = Object.keys(subjects);
    for (const key in quiz) {
        if (key === 'quizSubject') {
            if (!subjectsArray[quiz[key]]) throw 'Invalid Subject';
        } else if (key === 'quizDifficulty') {
            if (parseFloat(quiz[key]) < 1 || parseFloat(quiz[key]) >= 8)
            ...
            if (!isValidHttpUrl(quiz[key])) throw 'Invalid image url';
            ...
            if (languages.includes(quiz[key])) throw 'Invalid Language';
        }
    }
};
