...
const quizVerifier = (quiz) => {
	...
	try {
		for (const key in quiz) {
			if (key === 'quizSubject') {
				if (!subjectsArray.some((e) => e === quiz[key])) {throw `Invalid Subject ${quiz[key]}`;}
			}
			else if (key === 'quizDifficulty') {
				if (parseFloat(quiz[key]) < 0 || parseFloat(quiz[key]) >= 8) {throw 'Invalid Difficulty';}
			}
				...
				if (!Array.isArray(quiz[key])) {throw 'Invalid Incorrect Answers (Must be an array)';}
				for (const answer of quiz[key]) {
					// @ts-ignore
					// eslint-disable-next-line no-constant-condition
					if (!typeof answer === 'string') {throw 'Invalid Incorrect Answer (Must be a string)';}
				}
			}
			else if (key === 'quizImage') {
				if (!isValidHttpUrl(quiz[key])) {throw `Invalid image url: '${quiz[key]}'`;}
			}
				...
				if (!languages.includes(quiz[key])) {throw `Invalid language ${quiz[key]}`;}
			}
		}
	}
	catch (e) {
