...
router.get("/", (req, res) => {
  res.render("import", {
    ...
    type: "quiz",
    message:
      "Remember that the columns MUST be subject, difficulty, question, correct_answer, incorrect_answers, [image], type, category, lang.",
  });
});
...
router.post("/", async (req, res) => {
  ...
  csv.forEach(async (row) => {
    ...
    if (row["image"]) quiz["quizImage"] = row["image"];
