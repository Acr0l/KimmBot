...
module.exports = {
	...
	async execute(interaction, profileData) {
		...
		try {
			...
			else if (!profileData.equipment.includes(currentItem.id)) throw 'item_not_equipped';
			// Remove item from equipment
			profileData.equipment = profileData.equipment.filter((item) => item !== currentItem.id);
			...
			// @ts-ignore
			if (typeof err !== 'string') return interaction.reply(iTranslate(guild, 'error', { ns: 'common' }));
			// @ts-ignore
			interaction.reply(iTranslate(guild, `${REJECTION_PATH}.${err}`, { ns: 'glossary' }));
		}
	},
};
